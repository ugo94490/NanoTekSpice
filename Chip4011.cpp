...
Tristate Chip4011::compute(std::size_t pin)
{
    ...
    switch (pin) {
        case 3:
            value.at(3) = Compute::computeNand(address.find(1) != address.end() ? address.at(1).first.compute(address.at(1).second) : value.at(1),
            address.find(2) != address.end() ? address.at(2).first.compute(address.at(2).second) : value.at(2));
            return (value.at(3));
        case 4:
            value.at(4) = Compute::computeNand(address.find(5) != address.end() ? address.at(5).first.compute(address.at(5).second) : value.at(5),
            address.find(6) != address.end() ? address.at(6).first.compute(address.at(6).second) : value.at(6));
            return value.at(4);
        case 10:
            value.at(10) = Compute::computeNand(address.find(8) != address.end() ? address.at(8).first.compute(address.at(8).second) : value.at(8),
            address.find(9) != address.end() ? address.at(9).first.compute(address.at(9).second) : value.at(9));
            return value.at(10);
        case 11:
            value.at(11) = Compute::computeNand(address.find(12) != address.end() ? address.at(12).first.compute(address.at(12).second) : value.at(13),
...
bool Chip4011::checkLinkable(std::size_t pin)
{
...
void Chip4011::dump() const
{
}

void Chip4011::setValue(Tristate const &val, size_t pin) {
    value.at(pin) = val;
}