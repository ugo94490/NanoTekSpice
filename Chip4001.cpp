...
Tristate Chip4001::compute(std::size_t pin)
{
    ...
    switch (pin) {
        case 3:
            value[3] = Compute::computeNor(address.find(1) != address.end() ? address[1].first.compute(address[1].second) : value[1],
            address.find(2) != address.end() ? address[2].first.compute(address[2].second) : value[2]);
            return (value[3]);
        case 4:
            value[4] = Compute::computeNor(address.find(5) != address.end() ? address[5].first.compute(address[5].second) : value[5],
            address.find(6) != address.end() ? address[6].first.compute(address[6].second) : value[6]);
            return value[4];
        case 10:
            value[10] = Compute::computeNor(address.find(8) != address.end() ? address[8].first.compute(address[8].second) : value[8],
            address.find(9) != address.end() ? address[9].first.compute(address[9].second) : value[9]);
            return value[10];
        case 11:
            value[11] = Compute::computeNor(address.find(12) != address.end() ? address[12].first.compute(address[12].second) : value[13],
