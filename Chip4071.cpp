...
Tristate Chip4071::compute(std::size_t pin)
{
    ...
    switch (pin) {
        case 3:
            ...
            return (value.at(3));
        case 4:
            value.at(4) = Compute::computeOr(address.find(5) != address.end() ? address.at(5).first.compute(address.at(5).second) : value.at(5),
            address.find(6) != address.end() ? address.at(6).first.compute(address.at(6).second) : value.at(6));
            return value.at(4);
        case 10:
            value.at(10) = Compute::computeOr(address.find(8) != address.end() ? address.at(8).first.compute(address.at(8).second) : value.at(8),
            address.find(9) != address.end() ? address.at(9).first.compute(address.at(9).second) : value.at(9));
            return value.at(10);
        case 11:
            value.at(11) = Compute::computeOr(address.find(12) != address.end() ? address.at(12).first.compute(address.at(12).second) : value.at(13),
